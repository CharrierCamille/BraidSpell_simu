...
```{r}
data %>% filter(err==0 & nused!=2 & len>3 & len<9) %>% group_by(len) %>% sample_n(100,replace=FALSE)

data %>% filter(err==0 & nused!=2 & len>3 & len<9) %>% group_by(len) %>% sample_n(100,replace=FALSE) %>% select(c("item","len")) %>%  write.csv("../data/processed/flp_pseudowords_sampled.csv",row.names = FALSE)
```
