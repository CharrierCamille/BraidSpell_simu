---
title: "megalex_extracting_stim.Rmd"
...
path_megalex_visual = "../data/external/Megalex-items-visual.csv"

path_megalex_auditory = "../data/external/Megalex-items-auditory.csv"
...
megalex_auditory = read.csv(path_megalex_auditory,sep=";",fileEncoding="UTF-8")

megalex_visual = read.csv(path_megalex_visual,sep=";",fileEncoding="UTF-8")

megalex_auditory = megalex_auditory %>% inner_join(lexique_infra, by=c("target" = "word") )

megalex_visual = megalex_visual %>% inner_join(lexique_infra, by=c("target" = "word") )
```
...
```{r}
megalex_visual = megalex_visual %>% mutate(Freq_PG = as.numeric(gsub(",", ".", Freq_PG))) %>%
    ...
    mutate(class_cgp_min=as.factor(case_when(minfreqgraph_GP >=quantile(minfreqgraph_GP,0.75) ~ lab$high_consistency, 
                                            ...
                                            .default = "Mid")))%>% 
    filter(nchar(target)>=4 & nchar(target)<=8 & nchar(pron)>=4 & nchar(pron)<=8)

megalex_auditory = megalex_auditory %>% mutate(Freq_PG = as.numeric(gsub(",", ".", Freq_PG))) %>%
    mutate(class_cpg=as.factor(case_when(Freq_PG >=quantile(Freq_PG,0.75) ~ lab$high_consistency, 
                                            Freq_PG <=quantile(Freq_PG,0.25) ~ lab$low_consistency,
                                            .default = "Mid"))) %>%
    mutate(Freq_GP = as.numeric(gsub(",", ".", Freq_GP))) %>%
...
# stim CGP+ CPG+
stim1 = megalex_visual %>% filter(class_cgp==lab$high_consistency & class_cpg==lab$high_consistency) 

# stim CGP- CPG-
stim2 = megalex_visual %>% filter(class_cgp==lab$low_consistency & class_cpg==lab$low_consistency) 

# stim CGP+ CPG-
stim3 = megalex_visual %>% filter(class_cgp==lab$high_consistency & class_cpg==lab$low_consistency) 

# stim CGP+ CPG-
stim4 = megalex_visual %>% filter(class_cgp==lab$low_consistency & class_cpg==lab$high_consistency) 

stim_v = rbind(stim1,stim2,stim3,stim4) %>% select(-c(class_cgp_min, class_cpg_min))


# stim CGP+ CPG+
stim1 = megalex_auditory %>% filter(class_cgp==lab$high_consistency & class_cpg==lab$high_consistency) 

# stim CGP- CPG-
stim2 = megalex_auditory %>% filter(class_cgp==lab$low_consistency & class_cpg==lab$low_consistency) 

# stim CGP+ CPG-
stim3 = megalex_auditory %>% filter(class_cgp==lab$high_consistency & class_cpg==lab$low_consistency) 

# stim CGP+ CPG-
stim4 = megalex_auditory %>% filter(class_cgp==lab$low_consistency & class_cpg==lab$high_consistency) 

stim_a = rbind(stim1,stim2,stim3,stim4) %>% select(-c(class_cgp_min, class_cpg_min))

# write.csv(stim_v ,"../data/processed/full_stim_v.csv",row.names = FALSE)

stim_v    %>% group_by(class_cgp,class_cpg) %>% summarise(n=n(), len = mean(nblettres.x), phlen = mean(nbphons), nsyll = mean(nbsyll), freq = mean(freq), Freq_GP=mean(Freq_GP), Freq_PG = mean(Freq_PG), voisorth =mean(voisorth), voisphon=mean(voisphon))

stim_a   %>% group_by(class_cgp,class_cpg) %>% summarise(n=n(), len = mean(nblettres), phlen = mean(nbphons), nsyll = mean(nbsyll), freq = mean(freq), Freq_GP=mean(Freq_GP), Freq_PG = mean(Freq_PG), voisorth =mean(voisorth), voisphon=mean(voisphon))

# write.csv(stim_a ,"../data/processed/full_stim_a.csv",row.names = FALSE)


stim_v %>% group_by(class_cgp) %>% summarise(n=n(), len = mean(nblettres.x), phlen = mean(nbphons), nsyll = mean(nbsyll), freq = mean(freq), Freq_GP=mean(Freq_GP), Freq_PG = mean(Freq_PG), voisorth =mean(voisorth), voisphon=mean(voisphon))

stim_v    %>% group_by(class_cpg) %>% summarise(n=n(), len = mean(nblettres.x), phlen = mean(nbphons), nsyll = mean(nbsyll), freq = mean(freq), Freq_GP=mean(Freq_GP), Freq_PG = mean(Freq_PG), voisorth =mean(voisorth), voisphon=mean(voisphon))
```